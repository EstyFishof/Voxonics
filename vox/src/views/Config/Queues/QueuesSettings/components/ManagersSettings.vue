<template>
  <div class="personal-info-wrap" :class="{'disabled-events': !$access.isEditDepAndQueue(userInfo, 'queue', getQueueInfo)}">
    <div class="wrap-head">
      <h2>Managers in Queue</h2>
    </div>
    <div class="top">
      <div class="left" :class="{'disabled-gray-options-block': !$access.isEditDepAndQueue(userInfo, 'queue', getQueueInfo)}">Add manager
      </div>
      <div class="managers users">
        <div v-if="getManagersQueueInfo.length" class="manager user">
          <div class="block">
        ...
        <div v-if="getManagersQueueInfo.length > 1" class="manager user">
          <div class="block">
            <div class="round">Name</div>
            <span class="name"></span>
          </div>
        ...
        <div v-if="getManagersQueueInfo.length > 2" class="manager user">
          <div class="block">
            <div class="round">Name</div>
            <span class="name"></span>
          </div>
    ...
    <div class="wrap-body" :class="{'disabled-gray-options-block': !$access.isEditDepAndQueue(userInfo, 'queue', getQueueInfo)}">
      <div class="personal-info">
        <div class="personal-info__first-part">
          <div class="item-container with-err">
            <div class="label" v-click-outside="hideListUsers" ref="hideList">
              ...
              <DropdownList v-show="showManagers" :classNameList="'list'" :dataList="mainList.getMainList"  @scrollFunc="(event) => funScroll(event)"
                  ...
                  :classNameItemComputed="(item) => getManagersQueueInfo.filter(element => element.uid === item.uid).length ? collapse ? 'selected collapsed row' : 'selected row' : 'row'"
                  :showItem="(item) => getManagersQueueInfo.filter(element => element.uid === item.uid).length ? !collapse : true">
      ...
      <div class="managers users">
        <div class="manager user" v-for="(manager, index) of getManagersQueueInfo" :key="index">
          <div class="block">
          ...
          <div class="action">
            <label class="label-container">
              <input @change="changeEdit(manager)" :disabled="!$access.isEditDepAndQueue(userInfo, 'queue', getQueueInfo)" class="edit" v-model="manager.option.edit" type="checkbox"/>
              <span class="check"></span>
              <span class="text">Edit</span>
            </label>
          </div>
          <img @click="chooseAgent(manager)" class="union" src="@/assets/img/icons/union.svg" width="19" alt="" :class="{'disabled-events': !$access.isEditDepAndQueue(userInfo, 'queue', getQueueInfo)}">
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  watch: {
    getGlobalUsersList() {
      dropdownListFetch(this, this.getGlobalUsersList, 'getManagersQueueInfo')
      this.mainList.getMainList = _.concat(this.mainList.topList, this.mainList.bottomList)
    }
  },
  computed: {
    ...mapGetters(['getManagersQueueInfo', 'userInfo', 'getQueueInfo', 'getGlobalUsersList', 'getPaginatorUsers']),
  },
  methods: {
    searchDepartments() {
    ...
    chooseAgent(managerArg) {
      ...
      dropdownChooseItem(this, JSON.parse(JSON.stringify({...manager, option: {edit: false}})), 'getManagersQueueInfo', 'setAgentsDepartment')
    },
    changeEdit() {
      this.setAgentsDepartment()
    },
    setAgentsDepartment(array = this.getManagersQueueInfo) {
      this.$store.dispatch('setMagagersQueueInfo', array)
    }
  },
  mounted() {
    ...
    store.subscribe((mutation) => {
      if (mutation.type === 'queueInfoMutation') {
        this.mainList.topList = JSON.parse(JSON.stringify(this.getManagersQueueInfo))
      }
    })
    this.mainList.topList = JSON.parse(JSON.stringify(this.getManagersQueueInfo))
  },

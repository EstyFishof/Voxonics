  ...
  <div class="personal-info-wrap">
    ...
    <div class="top">
      ...
      <div class="managers users">
        <div v-if="newDepartmentManagers.length" class="manager user">
          <div class="block">
        ...
        <div v-if="newDepartmentManagers.length > 1" class="manager user">
          <div class="block">
            <div class="round">Name</div>
            <span class="name"></span>
          </div>
        ...
        <div v-if="newDepartmentManagers.length > 2" class="manager user">
          <div class="block">
            <div class="round">Name</div>
            <span class="name"></span>
          </div>
    ...
    <div class="wrap-body">
          ...
          <div class="item-container with-err">
            <div class="label" v-click-outside="hideListUsers">
              ...
              <DropdownList v-show="showManagers" :classNameList="'list'" :dataList="mainList.getMainList"  @scrollFunc="(event) => funScroll(event)"
                  ...
                  :classNameItemComputed="(item) => newDepartmentManagers.filter(element => element.uid === item.uid).length ? collapse ? 'selected collapsed row' : 'selected row' : 'row'"
                  :showItem="(item) => newDepartmentManagers.filter(element => element.uid === item.uid).length ? !collapse : true">
      ...
      <div class="managers users">
        <div class="manager user" v-for="(manager, index) of newDepartmentManagers" :key="index">
          <div class="block">
...
export default {
  ...
  watch: {
    getGlobalUsersList() {
      dropdownListFetch(this, this.getGlobalUsersList, 'newDepartmentManagers')
      this.mainList.getMainList = _.concat(this.mainList.topList, this.mainList.bottomList)
    }
  },
  computed: {
    ...mapGetters(['newDepartmentManagers', 'userInfo', 'getGlobalUsersList', 'getPaginatorUsers']),
  },
  methods: {
    searchDepartments() {
    ...
    chooseAgent(managerArg) {
      dropdownChooseItem(this, JSON.parse(JSON.stringify({...managerArg, option: {edit: false}})), 'newDepartmentManagers', 'setAgentsDepartment')
    },
    changeEdit() {
      this.setAgentsDepartment()
    },
    setAgentsDepartment(array = this.newDepartmentManagers) {
      this.$store.dispatch('transferManagersDepartment', array)
    }
  },
  mounted() {
    this.popupItem = this.$el.children[2].children[0]
    this.userInfo.option = {edit: true}
    this.$store.dispatch('transferManagersDepartment', [])
    if(this.userInfo.internalNumber && this.userInfo.internalNumber !== 100 && this.userInfo.internalNumber !== 0) {
      ...
      this.$store.dispatch('transferManagersDepartment', this.mainList.topList)
    }
    if (this.newDepartmentManagers.length) {
      this.mainList.topList = this.newDepartmentManagers
    }
  },
  directives: {
    ClickOutside

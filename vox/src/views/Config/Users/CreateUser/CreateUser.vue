<template>
  <div class="wrap-main">
    ...
    <div class="wrap-form">
      <!-- <form @submit.prevent> -->
        <PersonalInfo :requiredFields="requiredFields"/>
        <AgentSetup/>
        <QSettings/>
        <ManagerSettings/>
        <div class="btn-container">
          <button class="btn-submit" @click="checkValid(); handlerSubmit()">Apply Changes</button>
        </div>
      <!-- </form> -->
    </div>
  </div>
...
import ManagerSettings from "@/views/Config/Users/CreateUser/components/ManagerSettings/ManagerSettings"
...
import { parseAgent } from "@/lib/parse.user"


export default {
  name: "CreateUser",
  components: {QSettings, ManagerSettings, AgentSetup, PersonalInfo},
  data() {
    return {
      requiredFields: {
        name: false,
  ...
  computed: {
    ...mapGetters(['getForm', 'theme', 'getGlobalDepartments', 'getGlobalQueuesList'])
  },
  methods: {
    checkValid() {
      Object.keys(this.requiredFields).forEach(element => {
        this.requiredFields[element] = this.getForm[element] === ''
      })
    },
    async handlerSubmit() {
      const form = JSON.parse(JSON.stringify(this.getForm))//избавляемся от обзерверов
      const user = form.permission.role === 'agent' ? parseAgent(form) : form
      await UsersEffectsApi.createUser(user)

  ...
  <div class="get-users-page-container">
    <div class="toolbar">
      <div>
        <div class="btn-img-wrapper">
          <div id="batch-actions-block" class="batch-actions-block">
            <svg
              content="Batch action"
              @click="clickedUsers.length >= 2 ? setShowBatchActions() : null"
              v-tippy="{ placement: 'top', arrow: true, delay: 800, duration: 500 }"
              :class="{ batch: clickedUsers.length >= 2 }"
              class="btn-img"
              width="41"
              height="42"
              viewBox="0 0 41 42"
          ...
          <div
            ...
            v-tippy="{ placement: 'top', arrow: true, delay: 800, duration: 500 }"
            @click="goToAddNewUserPage"
          >
            <img v-if="theme === 'dark'" src="../../../assets/img/icons/add-user.svg" alt="add-user"/>
            <span v-else content="Add new department" class="add-btn-dep">
              <img height="15" src="@/assets/img/icons/plus.svg" alt="plus" class="add-btn-plus"/>
              <img
                width="28"
                height="32"
                src="@/assets/img/icons/add-user-without-shadow.svg"
                alt="plus"
      ...
      <Paginator
        v-if="getPaginatorUsers.paginator.recordsPerPage" perPage="8"
        :paginator="getPaginatorUsers.paginator"
        :methods="{ page: 'setUserPage', peerPage: 'setUserPerPage' }"
      />
    </div>
    ...
    <div class="data-container">
      <div v-if="users.length">
        <batch-actions
          v-if="clickedUsers.length >= 2 && showBatchActions"
          :batchType="'user'"
          ...
          :dataView="{
            ...
            textField: {
              field: 'position',
              textTitle: 'Update company positions',
              placeholder: 'Enter company positions'
            },
            listField: true,
            selectedText: 'users'
          }"
          @updateUsers="beforeUpdateUsers"
          ref="batchActions"
          v-click-outside="hideBatchActions"
        />
        <table class="user-table">
          <thead class="table-header">
          <tr>
            <th colspan="2" @click="sort('name')" :class="{ active: sortBy === 'name' }">
              <div class="clicked-head-block-for-barch-actions">
                <div
                  content="Mark all"
              ...
              Name <img :src="sortArrow" ref="name" alt=""/>
            </th>

            <th @click="sort('internalNumber')" :class="{ active: sortBy === 'internalNumber' }">
              Agent Number <img :src="sortArrow" ref="internalNumber" alt=""/>
            </th>

            <th class="departments-th">
              Departments
            </th>
            <th @click="sort('role')">Role <img :src="sortArrow" ref="role" alt=""/></th>

            <th @click="sort('position')" :class="{ active: sortBy === 'position' }">
              Company Position
              <img :src="sortArrow" ref="position" alt=""/>
            </th>

            <th class="email-th" @click="sort('email')" :class="{ active: sortBy === 'email' }">
              Email <img :src="sortArrow" ref="email" alt=""/>
            </th>
          ...
          <tbody>
          <tr v-for="user in users" :key="user.internalNumber">
            <td
              :class="{ 'marked-for-barch-actions': clickedUsers.filter(item => item.uid === user.uid).length }"
              class="ava-wrap"
            >
              <div class="clicked-hover-for-barch-actions">
                <div
                  content="Mark"
                  v-tippy="{ placement: 'top', arrow: true, delay: 500, duration: 500 }"
                  @click.stop="mark(user, clickedUsers)"
                  class="clicked"
                ></div>
              </div>
              <div class="ava">
                <div v-if="user.avatar" class="circle download"><img :src="user.avatar" alt=""/></div>
                <div v-if="!user.avatar" class="circle">
                  <img :src="ava" alt="user-avatar"/>
                </div>
              </div>
            </td>
            <td @click.stop="viewUser($event, user)" class="user-name">{{ user.name.slice(0, 14) || '' }}</td>
            <td @click.stop="viewUser($event, user)" class="left user-internal-number">
              {{ user.internalNumber || '' }}
            </td>
            <td @click.stop="viewUser($event, user)" class="user-dep">
              <div v-for="(department, index) in user.relatedWith.department.slice(0, 1)" :key="index + '0'">
                <span class="ellipsis department-name">{{ department.name }}</span>
                <div class="total">
                  total department:<br/>
                  <span>{{ user.relatedWith.department.length }}</span>
                </div>
              </div>
              <div v-if="!user.relatedWith.department.length"></div>
            </td>
            <td @click.stop="viewUser($event, user)" class="user-role">{{ user.permission.role || '' }}</td>
            <td @click.stop="viewUser($event, user)" class="user-position">{{ user.position || '' }}</td>
            <td @click.stop="viewUser($event, user)" class="user-email">{{ user.email || '' }}</td>
            <td class="actions">
              <label 
                class="action"
              >
...
import ava from '@/assets/img/user-solid.svg'
...
import { parseArrayOfUsers } from '@/lib/parse.user.js'
import ClickOutside from 'vue-click-outside'

export default {
  name: 'Users',
  components: {Paginator, BatchActions},
  data() {
    return {
      unsubscribeStore: () => {
      },
  ...
  watch: {
    getGlobalUsersList() {
      this.users = JSON.parse(JSON.stringify(this.getGlobalUsersList))
    }
  },
  computed: {
    ...mapGetters(['getPaginatorUsers', 'getGlobalUsersList', 'getGlobalSearch', 'theme'])
  },
  methods: {
    hideBatchActions(e) {
      let flag = false
      for (let value of e.path) {
        if (value.id === 'batch-actions-block' || value.id === 'batch-actions-main') flag = true
      }
      if (!flag) this.showBatchActions = false
    },
    block(user) {
      UsersEffectsApi.blockUser(user.uid, !user.onBlock)
        .then(() => user.onBlock = !user.onBlock)
    },
    callIn(user) {
      UsersEffectsApi.updateUser({callIn: !user.callIn}, user.uid)
        .then(res => user.callIn = res.callIn)
    },
    callOut(user) {
      UsersEffectsApi.updateUser({callOut: !user.callOut}, user.uid)
        .then(res => user.callOut = res.callOut)
    },
    sortByRole() {
      if (this.sortRole) {
        this.users.sort(function (a, b) {
          return ('' + a.permission.role).localeCompare(b.permission.role)
        })
      } else {
        this.users.sort(function (a, b) {
          return ('' + b.permission.role).localeCompare(a.permission.role)
        })
      }
      this.sortRole = !this.sortRole
    },
    setShowBatchActions() {
      this.showBatchActions = !this.showBatchActions
    },
    mark(user, users) {
      this.clickedUsers = markObj.mark(user, users)
    },
    markAll() {
      this.clickedUsers = markObj.markAll(this.users, this.clickedUsers)
    },
    beforeUpdateUsers(usersArray, option) {
      this.confirmationConfig.textConfirmation = 'Successfully updated'
      this.successUpdateBool = true
      this.usersToInfoModule = []
      usersArray.forEach(item => {
        if (item.relatedWith.department.length === 0) {
          this.usersToInfoModule.push(item.internalNumber)
          this.successUpdateBool = false
        }
      })
      if (this.usersToInfoModule.length) {
        this.confirmationConfig.textConfirmation = 'Users must be in at least one department, so they were added to the main department'
        this.$store.commit('confirmationMutation', this.confirmationConfig)
      }
      this.updateUsers(usersArray, option)
    },
    updateUsers(usersArray, option) {
      let array = usersArray
      if (array.length > 0) {
        this.$store.dispatch('setSpinner', true)
        const user = JSON.parse(JSON.stringify(array[0]))
        if (option === 'status') {
          UsersEffectsApi.blockUser(user.uid, user.onBlock)
            .then(response => {
              array.length === 1 && this.successUpdateBool ? (this.$store.commit('confirmationMutation', this.confirmationConfig)) : null
              array.shift()
              this.changeUsers({...response, uid: user.uid}, array, option)
              this.clickedUsers.shift()
            })
            .catch(() => (this.successUpdateBool = false))
        } else {
          UsersEffectsApi.updateArrayOfUsers(parseArrayOfUsers(user))
            .then(response => {
              array.length === 1 && this.successUpdateBool ? (this.$store.commit('confirmationMutation', this.confirmationConfig)) : null
              array.shift()
              this.clickedUsers.shift()
              this.changeUsers(response, array, option)
            })
            .catch(() => (this.successUpdateBool = false))
        }
    ...
    changeUsers(response, array, option) {
      for (let i = 0; i < this.users.length; i++) {
        if (this.users[i].uid === response.uid) {
          Object.keys(this.users[i]).map(item => {
            this.users[i][item] = response[item]
          })
        }
      }
      this.updateUsers(array, option)
    },
    searchUsers() {
      ...
      this.timer = setTimeout(
        () =>
          this.fetch(
            this.getPaginatorUsers.peerPage,
            this.sortBy,
            this.getPaginatorUsers.page,
            this.filter.name
          ),
        800
      )
    ...
    goToAddNewUserPage() {
      this.$router.push('/config/users/create-user')
    },
    sort(string) {
      ...
      return this.sortBy === string
    },
    fetch(peerPage, sort, page, search, order) {
      this.$store.dispatch('globalUsersListRequest', {peerPage, sort, page, search: this.filter.name, order})
    },
    async viewUser(e, user) {
      if (e.target.className !== 'action' || e.target.className !== 'clicked') {
        await UsersGettersApi.viewUser(user.uid)
        sessionStorage.setItem('userUid', user.uid)
        await this.$store.dispatch('toggleEditMode', true)
        return this.$router.push(`/config/users/account-info/${user.internalNumber}`)
      } else {
        return null
      }
    }
  ...
  mounted() {
    ...
    this.unsubscribeStore = this.$store.subscribe(mutation => {
      if (observables.paginator.users.includes(mutation.type)) {
        this.fetch(
          this.getPaginatorUsers.peerPage,
          this.sortBy,
          this.getPaginatorUsers.page,
    ...
    this.popupItem = this.$refs.batchActions
  },
  beforeDestroy() {
...
table {
  ...
  tbody {
    ...
    border-right: 2px solid var(--cdr-border-row) !important;
    border-bottom: 2px solid var(--cdr-border-row) !important;
    ...
    .actions {
      ...
      input ~ span {
        ...
        height: 11px;
        right: 20px;
        background-color: rgba(113, 116, 131, 0.5);
        border-radius: 8px;
      ...
      input:checked ~ span {
        ...
        width: 27px;
        height: 11px;
        background-color: rgba(39, 104, 153, 0.5);
        border-radius: 8px;
        right: 20px;

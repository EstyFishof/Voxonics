    ...
    <div class="body">
      <span ref="clickOutside" style="display: flex">
        ...
        <ChooseItemSchemeComponent
          ...
          currentSchemeType="announcement"
          currentSchemeTitle="Announcement"
          :ivrKey="getIVRKey"
        ...
        >
          ...
          <template v-slot:config>
            <FileBlock
              title="Add a new audio file"
              :valueFromStore="getItemScenario.options.filename"
              @updateItem="updateItem"
              propertyName="options.filename"
            />

            <SwitchBlock
              title="Change skip"
              :valueFromStore="getItemScenario.options.skip"
              @updateItem="updateItem"
              propertyName="options.skip"
            />
          </template>
        </ChooseItemSchemeComponent>
  ...
  methods: {
    ...
    updateItem(param, value) {
      this.$store.dispatch(
        ...
        _.set({...this.getItemScenario, schema: 'announcement'}, param, value)
      )
    }
  }
}

<template>
  <div class="table-main">
    ...
    <div class="table-outBound-calls-wrapper" @click="hideListOptions($event); indexRoute='false'"
         v-click-outside="hide">
      <div class="right-tabs">
        <ControlPanel :show="showApplyBtn" :apply="setApply" :discard="clearChanges"/>
      </div>
      <div class="toolbar">
        <div>
          <div class="batch-actions-block">
            <svg @click="showBatchActions = !showBatchActions; indexRoute = `batchActions`"
                 :class="{batch: clickedUsers.length >= 2}" class="btn-img" width="41" height="42"
              ...
              <path
                ...
                fill="#276899"/>
              <path :class="{batch: clickedUsers.length >= 2}"
            ...
            <span class="batch-actions-counter" v-if="clickedUsers.length"
                  :class="{'batch-actions-max-counter': clickedUsers.length === 12}">{{ clickedUsers.length }}</span>
          </div>
          <ul class="sort-tabs">
            <li class="filter-text">Filter by:</li>
            <li :ref="`filterUsersName`" @click="indexRoute = `filterUsersName`">
              <span class="options-label">
                <label>
                  <input class="input-field num" type="text"
                         :value="filters.user && filters.user.name || 'Users'"
                         @input="search($event.target.value)" @click="isSearch = 'filterUsersName'">
                  <span class="triangle"
                        @click="setShowList('showList', `filterUsersName`, 'users', $event.target.parentNode.parentNode.getElementsByClassName('users')[0])"
                        :class="{'rotate-arrow': showList === 'filterUsersName'}">
                    <svg width="12" height="7" viewBox="0 0 12 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path opacity="0.9" d="M1 1L6 6L11 1" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                  </span>
                </label>
                <DropdownList v-show="showList === 'filterUsersName'"
                  :classNameList="'list-related-numbers routing-filter-dropdown-list users'"
                  :dataList="getGlobalUsersList"
                  @clickFunc="(item) => {setFilterProps('user', {name: item.name, number: item.internalNumber}); filtersFunc(item.internalNumber)}"
                  :itemText="(item) => `<span class='name'>${ item.name }</span><span class='number'> ${ item.internalNumber }</span>`"
                  :classNameInput="'list-related-input'"
                  :valueInput="searchValue"
                  :clickFuncAll="() => {setFilterProps('user', null); filtersFunc('')}"
                  :contentTippyCondition="(item) => item.name.length > 6"
                  @changeInput="(event) => search(event.target.value)"
                  :classNameItemComputed="element => filters.user && filters.user.number === element.internalNumber ? 'choosed' : ''"
                />
              </span>
            </li>
            <li :ref="`filterDepartment`" @click="indexRoute = `filterDepartment`">
              <span class="options-label">
                <label>
                  <input class="input-field num" type="text"
                         :value="filters.department && filters.department.name || 'Department'"
                         @input="search($event.target.value)" @click="isSearch = 'filterDepartment'">
                  <span class="triangle"
                        @click="setShowList('showList', `filterDepartment`, 'department', $event.target.parentNode.parentNode.getElementsByClassName('department')[0])"
                        :class="{'rotate-arrow': showList === 'filterDepartment'}">
                    <svg width="12" height="7" viewBox="0 0 12 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path opacity="0.9" d="M1 1L6 6L11 1" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                  </span>
                </label>
                <DropdownList v-show="showList === 'filterDepartment'"
                  :classNameList="'list-related-numbers routing-filter-dropdown-list department'" :dataList="getGlobalDepartments"
                  @clickFunc="(item) => setFilterProps('department', {name: item.name, uid: item.uid})"
                  :itemText="(item) => item.name"
                  :classNameInput="'list-related-input'" :valueInput="searchValue"
                  :clickFuncAll="() => {setFilterProps('department', null); filtersFunc('')}"
                  :contentTippyCondition="(item) => item.name.length > 10"
                  @changeInput="(event) => search(event.target.value)"
                  :classNameItemComputed="element => filters.department && filters.department.uid === element.uid ? 'choosed' : ''"
                />
              </span>
            </li>
            <li :ref="`filterDirection`" @click="indexRoute = `filterDirection`">
              <span class="options-label">
                <label>
                  <input class="input-field num" type="text"
                         :value="filters.direction ? `${filters.direction.code}(${filters.direction.type})` : 'Direction'"
                         @input="search($event.target.value)" @click="isSearch = 'filterDirect'">
                  <span class="triangle"
                        @click="setShowList('showList', `filterDirection`, 'direction', $event.target.parentNode.parentNode.getElementsByClassName('direction')[0])"
                        :class="{'rotate-arrow': showList === 'filterDirection'}">
                    <svg width="12" height="7" viewBox="0 0 12 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path opacity="0.9" d="M1 1L6 6L11 1" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                  </span>
                </label>
                <DropdownList v-show="showList === 'filterDirection'" :classNameList="'list-related-numbers routing-filter-dropdown-list direction'"
                  :dataList="getDirections"
                  @clickFunc="(item) => setFilterProps('direction', {code: item.code, type: item.type, uid: item.uid})"
                  :clickFuncAll="() => {setFilterProps('direction', null); filtersFunc('')}"
                  :itemText="(item) => `${item.code}(${item.type})`"
                  :classNameInput="'list-related-input'" :valueInput="searchValue"
                  :contentTippyCondition="(item) => `${item.code}(${item.type})`.length < 10"
                  @changeInput="(event) => search(event.target.value)"
                  :classNameItemComputed="element => filters.direction && filters.direction.uid === element.uid ? 'choosed' : ''"
                />
              </span>
            </li>
            <li :ref="`filterExternalNumber`" @click="indexRoute = `filterExternalNumber`">
              <span class="options-label">
                <label>
                  <input class="input-field num" type="text"
                         :value="filters.externalNumber ? filters.externalNumber.number : 'V-number'"
                         @input="search($event.target.value)" @click="isSearch = 'filterNumber'">
                  <span class="triangle"
                        @click="setShowList('showList', `filterExternalNumber`, 'externalNumber', $event.target.parentNode.parentNode.getElementsByClassName('externalNumber')[0])"
                        :class="{'rotate-arrow': showList === 'filterExternalNumber'}">
                    <svg width="12" height="7" viewBox="0 0 12 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path opacity="0.9" d="M1 1L6 6L11 1" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                  </span>
                </label>
                <DropdownList v-show="showList === 'filterExternalNumber'"
                  :classNameList="'list-related-numbers routing-filter-dropdown-list externalNumber external-number-description-list'"
                  :dataList="getGlobalExternalNumbers"
                  @clickFunc="(item) => setFilterProps('externalNumber', {number: item.number, uid: item.uid})"
                  :clickFuncAll="() => {setFilterProps('externalNumber', null); filtersFunc('')}"
                  :itemText="(item) => `${item.number}<span class='description'>${item.description}</span>`"
                  :contentTippyCondition="(item) => item.number.length > 10"
                  :classNameItemComputed="element => filters.externalNumber && filters.externalNumber.uid === element.uid ? 'choosed' : ''"
                />
              </span>
            </li>
            <li class="allowed-filter" :ref="`filterAllowed`" @click="indexRoute = `filterAllowed`">
              <div class="allowed-filter-item" :class="{'not-open-allowed-filter': showList !== 'filterAllowed'}">
                ...
                <svg class="allowed-filter-arrow" :class="{'allowed-filter-arrow-down': showList !== 'filterAllowed'}"
                     width="5" height="8" viewBox="0 0 5 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M4 4L1 1V7L4 4Z" fill="var(--allowed-filter-arrow)" stroke="var(--allowed-filter-arrow)"
            ...
            <li class="clear-filters-list-item" v-if="showClearAllFiltersBtn">
              <span class="clear-filters-block" content="Clear all" @click="clearAllFilters()"
                    v-tippy="{ placement : 'top',  arrow: true, delay : 800, duration: 500 }">
      ...
      <div class="table-wrapper">
        ...
        <div class="table-header">
          <span :ref="`batchActions`" @click="indexRoute = `batchActions`">
            <batch-actions v-if="clickedUsers.length > 1 && showBatchActions" :batchType="'routes'"
                           :arrayOfUsers="clickedUsers"
                           :dataView="{togglesField: true, textField: {field: 'position', textTitle: 'Update company positions', placeholder: 'Enter company positions'}, listField: true, selectedText: 'users'}"
                           @updateUsers="(usersToUpdate) => updateUsers(addDefaultRouteBeforeUpdateUsers(usersToUpdate))"/>
          </span>

          <div class="title-and-paginator">
            <span class="title">Outbound Routing</span>

            <Paginator class="paginator" v-if="paginatorProp.paginator.paginator.recordsPerPage" perPage="12"
                       :paginator="paginatorProp.paginator.paginator"
                       :methods="{page: paginatorProp.methods.page, peerPage: paginatorProp.methods.perPage}"/>
          </div>

          <div class="header-list-block">
            <ul class="header-list">
              <li class="header-list-item item-name" @click="sortFunc('name', 'sortName')">
                <div class="clicked-head-block-for-barch-actions" content="Mark all"
                     v-tippy="{ placement : 'top',  arrow: true, delay : 800, duration: 500 }">
                  <div @click.stop="markAll()" class="clicked-head"></div>
                </div>
                Name
                <img ref="sortName" src="@/assets/img/icons/sort-arrow.svg"/>
              </li>
              <li class="header-list-item item-internal-number"
                  @click="sortFunc('internalNumber', 'sortInternalNumber')">
                Agent Number
                <img ref="sortInternalNumber" src="@/assets/img/icons/sort-arrow.svg"/>
              </li>
              <li class="header-list-item item-dep">
                Department
                <img ref="sortDepartment" src="@/assets/img/icons/sort-arrow.svg"/>
                <!-- <img class="question" src="@/assets/img/icons/question-mark-white.svg"/> -->
              </li>
        ...
        <div class="table-body">
          <ul class="table-main-list">
            <li class="table-main-list-item" v-for="(item, index) in listRoutes" :key="index">
              <div class="list-block">
                <div class="clicked-hover-for-barch-actions" @click="indexRoute = `batchActions`" content="Mark"
                     v-tippy="{ placement : 'top',  arrow: true, delay : 800, duration: 500 }"
                     :class="{'marked-for-barch-actions': clickedUsers.filter(element => element.uid === item.uid).length}">
                  <div @click.stop="mark(item, clickedUsers)" class="clicked"></div>
                </div>
                <ul class="horizontal-list" @click="toggleOpenList(item)">
                  <li class="horizontal-list-item item-name ellipsis">
                    {{ item.name }}
                  </li>
                  <li class="horizontal-list-item item-internal-number">
                    {{ item.internalNumber }}
                  </li>
                  <li class="horizontal-list-item item-dep">
                    <!-- Sales...({{ item.departments }}) -->
                    Sales... ({{ item.departments.length }})
                    <img v-if="item.departments.length" @click.stop="showDepartments(item.uid)" width="16" height="16" src="@/assets/img/icons/question-mark.svg"/>

                    <div v-show="showRefers === item.uid" class="refers-wrap">
                      <span>This number refers to Departments:</span>
                      <ul>
                        <li @click="toDepartment(department)" v-for="department of item.departments"
                  ...
                  <li class="horizontal-list-item item-arrow">
                    <button content="Edit mode"
                            v-tippy="{ placement : 'top',  arrow: true, delay : 800, duration: 500 }"
                            @click.stop="editMode(item)" class="btn-edit"><img src="@/assets/img/icons/edit.svg" alt="">
                    </button>
                    <svg class="arrow right"
                         width="25" height="20" viewBox="0 0 10 18" fill="transparent"
                         :style="{transform: item.open ? 'rotate(270deg)' : 'rotate(180deg)'}"
                         xmlns="http://www.w3.org/2000/svg">
                      <path d="M9 1L1 9L9 17" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                  </li>
                </ul>
                <ul v-if="item.open" class="secondary-list">
                  <li class="secondary-list-item" v-if="item.edit">
                    <ul class="horizontal-secondary-list new-route-list">
                      <li class="btn-item">
                        <span class="add-route-btn-span" :class="{'disabled-btn': !validNewRoute(item)}"
                              @click="validNewRoute(item) ? createRoute(item) : item.newRoute.checkValid = true">
                          <span class="add-route-btn"><img src="@/assets/img/icons/add-route-btn.svg"/></span>
                        </span>
                      </li>
                      <li class="horizontal-secondary-list-item item-name ellipsis">{{ item.name }}</li>
                      <li class="horizontal-secondary-list-item item-internal-number">{{ item.internalNumber }}</li>
                      <li class="horizontal-secondary-list-item item-dep"></li>
                      <li class="horizontal-secondary-list-item item-direction" :class="{'red-line': item.newRoute.checkValid && !item.newRoute.direction.code}" :ref="`direction${item.uid}create`"
                          @click="indexRoute = `direction${item.uid}create`">
                        <span class="options-label">
                          <label>
                            <input class="input-field num" type="text"
                                   :value="item.newRoute.direction.code ? `${item.newRoute.direction.code}(${item.newRoute.direction.type})` : 'Select direction'"
                                   readonly>
                            <span class="triangle"
                          ...
                          <DropdownList v-show="showList === `direction${item.uid}create`"
                            :classNameList="'list-related-numbers direction'"
                            :dataList="getDirections.filter(dir => dir.code !== '00')"
                            @clickFunc="(element) => setParams(item.newRoute, 'direction', element)"
                            :itemText="(element) => `${element.code}(${ element.type })`"
                            :classNameInput="'list-related-input'" :valueInput="searchValue"
                            :contentTippyCondition="(element) => `${element.code}(${element.type})`.length > 10"
                            @changeInput="(event) => search(event.target.value)"
                            :classNameItemComputed="element => item.newRoute.direction.code === element.code ? 'choosed' : ''"
                          />
                        </span>
                      </li>
                      <li class="horizontal-secondary-list-item item-number" :class="{'red-line': item.newRoute.checkValid && externalNumberValue(item.newRoute) === ''}" :ref="`externalNumber${item.uid}create`"
                          @click="indexRoute = `externalNumber${item.uid}create`">
                        <label class="options-label">
                          <input class="input-field num" type="text"
                                 :value="externalNumberValue(item.newRoute) || 'Select V-num'">
                          ...
                          <ul v-show="showList === `externalNumber${item.uid}create`" class="list-related-numbers">
                            ...
                            <li @click="setVNumber(item.newRoute, 'preferred', true)">Preferred</li>
                          </ul>
                          <DropdownList v-show="showList === `externalNumber${item.uid}createNumber`"
                              :classNameList="'list-related-numbers externalNumber external-number-description-list'"
                              :dataList="getGlobalExternalNumbers"
                              @clickFunc="(element) => setVNumber(item.newRoute, 'externalNumber', element)"
                              :itemText="(element) => `${element.number}<span class='description'>${element.description}</span>`"
                              :classNameInput="'list-related-input'" :valueInput="searchValue"
                              :contentTippyCondition="(element) => element.number.length > 10"
                              @changeInput="(event) => search(event.target.value)"
                              :classNameItemComputed="element => item.newRoute.externalNumber && item.newRoute.externalNumber.uid === element.uid ? 'choosed' : ''"
                          />
                          <DropdownList v-show="showList === `externalNumber${item.uid}createDepartment`"
                              :classNameList="'list-related-numbers department'"
                              :dataList="getGlobalDepartments"
                              @clickFunc="(element) => setVNumber(item.newRoute, 'department', element)"
                              :itemText="(element) => element.name"
                              :classNameInput="'list-related-input'" :valueInput="searchValue"
                              :contentTippyCondition="(element) => element.name.length > 10"
                              @changeInput="(event) => search(event.target.value)"
                              :classNameItemComputed="element => item.newRoute.department && item.newRoute.department.uid === element.uid ? 'choosed' : ''"
                          />
                        </label>
                      </li>
                      <li class="horizontal-secondary-list-item item-priority" :ref="`allowed${item.uid}create`"
                          @click="indexRoute = `allowed${item.uid}create`">
                        <span class="allowed-span input-priority-span" :class="{'active-allowed': showList === `allowed${item.uid}create`, 'red-line': item.newRoute.checkValid && !item.newRoute.priority}">
                          <input class="input-priority"
                            @keydown="onKeyDownHandlerForPriorityInput"
                            maxlength="3"
                            @input="setParams(item.newRoute, 'priority', $event.target.value, 'number')"
                            :value="item.newRoute.priority || ''"
                            @focus="setShowList('showList', `allowed${item.uid}create`)"
                            @blur="setShowList('showList', '')"
                          />
                        </span>
                        <span class="status-span">
                          <span class="status-null" v-if="item.newRoute.allowed === null"
                              @click="item.newRoute.showAllowed = !item.newRoute.showAllowed"></span>
                          <img class="status-img" v-else
                              :src="item.newRoute.allowed ? successStatusPayment : redStatusPayment" alt=""
                              @click="item.newRoute.showAllowed = !item.newRoute.showAllowed">
                          <div class="status-block" v-if="item.newRoute.showAllowed">
                            <div class="status-container">
                              <img @click="item.newRoute.allowed = true; item.newRoute.showAllowed = false"
                                  :src="successStatusPayment"/>
                              <img @click="item.newRoute.allowed = false; item.newRoute.showAllowed = false"
                  ...
                  <li class="secondary-list-item" v-for="(el, indexEl) in item.route" :key="indexEl">
                    <ul class="horizontal-secondary-list">
                      <li v-if="item.edit && el.direction.code !== '00'" class="btn-item">
                        <span class="add-route-btn-span" @click="setDeleteRouteActive(item, el)">
                      ...
                      <li class="horizontal-secondary-list-item item-direction"
                          :class="{'non-events': el.direction.code === '00'}" :ref="`direction${el.uid}`"
                          @click="indexRoute = `direction${el.uid}`">
                        <span class="options-label">
                          <label>
                            <input class="input-field num" type="text"
                                   :value="el.direction.code === '00' ? 'Other directions' :`${el.direction.code}(${el.direction.type})`"
                                   readonly>
                            <span class="triangle"
                                  @click="setShowList('showList', `direction${el.uid}`, 'direction', $event.target.parentNode.parentNode.getElementsByClassName('direction')[0])"
                                  :class="{'rotate-arrow': showList === `direction${el.uid}`}">
                              <svg width="20" height="10" viewBox="0 0 12 7" fill="none"
                                   xmlns="http://www.w3.org/2000/svg">
                                <path opacity="0.9" d="M1 1L6 6L11 1" stroke-linecap="round" stroke-linejoin="round"/>
                              </svg>
                          ...
                          <DropdownList v-show="showList === `direction${el.uid}`"
                              :classNameList="'list-related-numbers direction'"
                              :dataList="getDirections.filter(dir => dir.code !== '00')"
                              @clickFunc="(element) => setParams(el, 'direction', element)"
                              :itemText="(element) => `${element.code}(${ element.type })`"
                              :classNameInput="'list-related-input'" :valueInput="searchValue"
                              :contentTippyCondition="(element) => `${element.code}(${element.type})`.length > 10"
                              @changeInput="(event) => search(event.target.value)"
                              :classNameItemComputed="element => el.direction.uid === element.uid ? 'choosed' : ''"
                          />
                        </span>
                      </li>
                      <li class="horizontal-secondary-list-item item-number" :ref="`externalNumber${el.uid}`"
                          @click="indexRoute = `externalNumber${el.uid}`">
                        <span class="options-label">
                          <label>
                            <input class="input-field num" type="text" :value="externalNumberValue(el)" readonly>
                            <span class="triangle" @click="setShowList('showList', `externalNumber${el.uid}`)"
                                  :class="{'rotate-arrow': showList === `externalNumber${el.uid}`}">
                              <svg width="20" height="10" viewBox="0 0 12 7" fill="none"
                                   xmlns="http://www.w3.org/2000/svg">
                                <path opacity="0.9" d="M1 1L6 6L11 1" stroke-linecap="round" stroke-linejoin="round"/>
                              </svg>
                          ...
                          <ul v-show="showList === `externalNumber${el.uid}`" class="list-related-numbers">
                            <li
                              @click="setShowList('showList', `externalNumber${el.uid}number`, 'externalNumber', $event.target.parentNode.parentNode.getElementsByClassName('externalNumber')[0])">V-number</li>
                            <li
                              @click="setShowList('showList', `externalNumber${el.uid}department`, 'department', $event.target.parentNode.parentNode.getElementsByClassName('department')[0])">Department</li>
                            <li @click="setVNumber(el, 'preferred', true)">Preferred</li>
                          </ul>
                          <DropdownList v-show="showList === `externalNumber${el.uid}number`"
                              :classNameList="'list-related-numbers externalNumber external-number-description-list'"
                              :dataList="getGlobalExternalNumbers"
                              @clickFunc="(element) => setVNumber(el, 'externalNumber', element)"
                              :itemText="(element) => `${element.number}<span class='description'>${element.description}</span>`"
                              :classNameInput="'list-related-input'" :valueInput="searchValue"
                              :contentTippyCondition="(element) => element.number.length > 10"
                              @changeInput="(event) => search(event.target.value)"
                              :classNameItemComputed="element => el.externalNumber && el.externalNumber.uid === element.uid ? 'choosed' : ''"
                          />
                          <DropdownList v-show="showList === `externalNumber${el.uid}department`"
                              :classNameList="'list-related-numbers department'"
                              :dataList="getGlobalDepartments"
                              @clickFunc="(element) => setVNumber(el, 'department', element)"
                              :itemText="(element) => element.name"
                              :classNameInput="'list-related-input'" :valueInput="searchValue"
                              :contentTippyCondition="(element) => element.name.length > 10"
                              @changeInput="(event) => search(event.target.value)"
                              :classNameItemComputed="element => el.department && el.department.uid === element.uid ? 'choosed' : ''"
                          />
                        </span>
                      </li>
                      <li class="horizontal-secondary-list-item item-priority">
                        <span class="allowed-span input-priority-span"
                              :class="{'active-allowed': showList === `priority${el.uid}`, 'non-events': el.direction.code === '00'}">
                          <input 
                            class="input-priority"
                            @keydown="onKeyDownHandlerForPriorityInput"
                            maxlength="3"
                            @input="setParams(el, 'priority', $event.target.value, 'number')" :value="el.priority"
                            @focus="setShowList('showList', `priority${el.uid}`)"
                            @blur="setShowList('showList', '')"
                          />
                        </span>
                        <img class="status-img" :src="el.allowed ? successStatusPayment : redStatusPayment" alt=""
                             @click="setShowList('showList', `allowed${el.uid}`); indexRoute = `allowed${el.uid}`"
                             :ref="`allowed${el.uid}`">
                        <div class="status-block" v-if="showList === `allowed${el.uid}`" :ref="`allowed${el.uid}`"
                             @click="indexRoute = `allowed${el.uid}`">
                          <div class="status-container">
                            <img @click="setParams(el, 'allowed', true)" :src="successStatusPayment"/>
                            <img @click="setParams(el, 'allowed', false)" :src="redStatusPayment"/>
                          </div>
                        </div>
                      </li>
                    </ul>
...
import { usersForLayout } from '../../../../lib/parse.Routes'

export default {
  name: 'TableOutBoundCalls',
  components: {Paginator, ControlPanel, BatchActions, DeleteComponent, DropdownList},
  props: ['listRoutesProp', 'paginatorProp', 'initialListRoutesProp', 'obj'],
  data() {
    return {
      filters: {
        user: null,
      ...
      confirmation: false,
      indexRoute: '',
      textToDeleteComponent: 'Are you sure you want to delete this Route ?',
  ...
  watch: {
    listRoutesProp() {
      this.listRoutes = this.listRoutesProp
    },
    initialListRoutesProp() {
      this.isEqual()
    }
  },
  computed: {
    ...mapGetters(['getDirections', 'getGlobalExternalNumbers', 'getGlobalDepartments', 'getGlobalUsersList']),
  ...
  methods: {
    showDepartments(uid) {
      if (this.showRefers === uid) {
        this.showRefers = null
      } else {
        this.showRefers = uid
      }
    },
    ...
    toggleOpenList(item) {
      if(!item.open) {
        this.$store.dispatch('getCurrentUserRoutes', {uid: item.uid, ...this.obj})
      }
      item.open = !item.open
      item.newRoute.checkValid = false
    },
    hide() {
      this.showList = ''
    ...
    validNewRoute(item) {
      return routingFunctional.validNewRoute(this, item.newRoute)
    },
    addDefaultRouteBeforeUpdateUsers(array) {

      const getDefaultRoute = (user) => user.route.filter(el => el.direction.code === '00')[0]

      return array.map(item => {
        if(!getDefaultRoute(item)) {
          this.clickedUsers.forEach(user => {
            if(user.uid === item.uid && getDefaultRoute(user)) {
              item.route.push(getDefaultRoute(user))
            }
          })
        }
        return item
    ...
    updateUsers(array) {     
      ...
      let bool = true
      array.map(item => {
        item.route = item.route.map(element => {
          element = parseRoutes(element)
          return element
        })
        item.route = _.sortBy(item.route, ['departmentUid', 'priority'])
        return item
      })
      const func = route => UsersEffectsApi.updateUserRoutes(route.uid, {route: route.route})
        .catch(e => {
          throw(e)
        })
      Promise.all(array.map(func))
        .then(() => bool ? this.$store.commit('confirmationMutation', this.confirmationConfig) : null)
        .catch(() => {
          bool = false
        })
      this.clickedUsers = []
    ...
    mark(user, users) {
      this.getRoutes(user.uid)
      this.clickedUsers = markObj.mark(user, users)
    },
    markAll() {
      this.clickedUsers = markObj.markAll(this.listRoutes, this.clickedUsers)
      this.$store.dispatch('getUsersWithoutRoutes', this.clickedUsers)
        .then(res => res.forEach(({uid}) => this.getRoutes(uid)))
        
    },
    getRoutes(uid) {
      this.$store.dispatch('getCurrentUserRoutes', {uid, ...this.obj})
    },
    filtersFunc(prop) {
      this.activeFilter = prop
      // console.log(this.activeFilter, prop)
      // UsersGettersApi.getUsersWithoutFullAppend(payload.config)
      this.$store.dispatch('getListOfUsersForOutboundRoutes', {
        config: {
          peerPage: this.paginatorProp.paginator.peerPage,
          page: this.paginatorProp.paginator.page,
          sort: this.activeSort,
          order: this.activeOrder,
          search: this.activeFilter
        },
        ...this.obj
      })
      // UsersGettersApi.getRoutes(this.obj, this.paginatorProp.paginator.peerPage, this.paginatorProp.paginator.page, this.activeFilter, this.activeSort, this.activeOrder)
    },
    sortFunc(sortParam, refParam) {
      if (this.sort[sortParam] === 'desc') {
        this.sort[sortParam] = 'asc'
        this.$refs[refParam] ? this.$refs[refParam].style.transform = 'rotate(180deg)' : null
      } else {
        this.sort[sortParam] = 'desc'
        this.$refs[refParam] ? this.$refs[refParam].style.transform = '' : null
      }
      this.activeSort = sortParam
      this.activeOrder = this.sort[sortParam]
      ...
      // UsersGettersApi.getUsersWithoutFullAppend({
      //   peerPage: this.paginatorProp.paginator.peerPage,
      //   page: this.paginatorProp.paginator.page,
      //   sort: this.activeSort,
      //   order: this.activeOrder,
      ...
      //     this.listRoutes = JSON.parse(JSON.stringify(data))
      //   })
      // UsersGettersApi.getRoutes(this.obj, this.paginatorProp.paginator.peerPage, this.paginatorProp.paginator.page, this.activeFilter, this.activeSort, this.activeOrder)
    },
    ...
    setDeleteRouteActive(item, el) {
      this.deleteRouteActive = {item: item.uid, el: el.uid}
      // this.textToDeleteComponent = `Are you sure you want to delete this Route for ${item.name} with direction ${el.direction.code}(${el.direction.type}) ?`,
      this.textToDeleteComponent = `Are you sure you want to delete this Route for ${item.name} ?`,
    ...
    deleteRoute() {
      UsersEffectsApi.deleteRoute(this.deleteRouteActive.item, this.deleteRouteActive.el)
      this.deleteRouteActive = {}
      this.showDeleteComponents = false
    },
    createRoute(item) {
      const obj = parseRoutes(item.newRoute)
      UsersEffectsApi.createRoute(item.uid, obj)
    },
    ...
    setApply() {
      const func = route => UsersEffectsApi.updateRoutes(route.uid, route.route.uid, parseRoutes(route.route), route.open)
      Promise.all(this.arrayToUpdate.map(func))
        .then(() => {
          this.arrayToUpdate = []
        })
    ...
    clearChanges() {
      this.$store.dispatch('getListOfUsersForOutboundRoutes', {...this.obj, config: {peerPage: this.paginatorProp.paginator.peerPage, page: this.paginatorProp.paginator.page}})
      // UsersGettersApi.getRoutes(this.obj, this.paginatorProp.paginator.peerPage, this.paginatorProp.paginator.page, this.activeFilter, this.activeSort, this.activeOrder)
    },
    isEqual() {
      let bool = false
      this.arrayToUpdate = []
      for (let i = 0; i < this.initialListRoutesProp.length; i++) {
        const item = JSON.parse(JSON.stringify(_.omit(this.listRoutes[i], ['open', 'edit', 'newRoute'])))
        // item.route.sort()
        item.route = _.sortBy(item.route, ['uid'])
        const initialItem = JSON.parse(JSON.stringify(_.omit(this.initialListRoutesProp[i], ['open', 'edit', 'newRoute'])))
        initialItem.route = _.sortBy(initialItem.route, ['uid'])

        if (!_.isEqual(initialItem, item)) {
          this.showApplyBtn = true
          bool = true
          for (let j = 0; j < this.initialListRoutesProp[i].route.length; j++) {
            if (!_.isEqual(this.initialListRoutesProp[i].route[j], this.listRoutes[i].route[j])) {
              this.arrayToUpdate.push(JSON.parse(JSON.stringify({
                uid: this.listRoutes[i].uid,
                route: this.listRoutes[i].route[j],
                open: this.listRoutes[i].open
              })))
            }
          }
        }
...
<style lang="scss" scoped src="@/assets/styles/outboundRouting/edit-btn-pen.scss"></style>
<style lang="scss">
.routing-filter-dropdown-list {
  li {
    margin-right: 0;
  }
}
...
.table-wrapper {
  .table-header {
    ...
    .title-and-paginator {
      ...
      justify-content: space-between;
      height: 62px;
      background: var(--outbound-routing-header-bg);
    ...
    .table-main-list {
      ...
      .table-main-list-item {
        ...
        &::before {
          ...
          width: 100%;
          height: 1px;
          background: var(--outbound-routing-line-over-item);
          top: -1px;
          left: 0;
        }
            ...
            .horizontal-list-item {
              ...
              &.item-arrow {
                ...
                .arrow {
                  ...
                  stroke: var(--outbound-routing-arrow);
                  fill: transparent;
          ...
          .secondary-list {
            ...
            .secondary-list-item {
              ...
              &::after {
                ...
                height: 1px;
                width: calc(100% - 20px);
                background: var(--outbound-routing-line-under-item);
              }
